utils.selectNamespace("utils.custom", function(custom){
    ...
    custom.iVal = function($widget, oMap, preventPropagationOrForce) {
        if (!$widget[0])
            return false;
        var oldVal = $widget[0].getAttribute('data-ival') || '';
        var ev = new custom.event('change', $widget, oMap);
        ev.oldVal = oldVal;
    ...
    custom.defineWidget = function(name, cb) {
        ...
        wSpace.on = function(selector, eventName, cb,ready) {
        	custom.readyEventQ.put(function(e){
        		return $("body").on(eventName,"div.tag." + this.widgetName + " " + selector, function(e,e2,e3,e4,e5){
                	var $this = $(this)
    ...
    custom.on = function(selector, eventName, cb) {
    	custom.readyEventQ.put(function(){
    		return $("body").on(eventName,selector, function(a,b,c,d,e){
            	return cb.call(this,a,b,c,d,e);
            });
    	}.bind(this))
    };
    custom.toggleOption = function(elem,filterClass,className,add){
	...
	$.fn.toggleOption = function(filterClass,className,add){
		return custom.toggleOption(this,filterClass,className,add);
	};
...
$.fn.setValue = function(iVal,attr,trigger, _dVal){
	return this.each(function(index,elem){
		...
		if(tagType){
			...
			} else utils.custom.iVal($widget, new utils.format.defMap(iVal ,_dVal));
		} else {
	...
	$widgets.attr("temp", _value);
	if(attr) $widgets.attr("attr", attr);
	var valObj = _value;
	var __retVal;
	if($widgets.hasClass("multiselect")){
		utils.custom.multiselect.setValue(this,_value,attr);
	} else {
		$widgets.each(function(index, element){
			...
			if(element.tagName==='INPUT'){
				...
				if(formatType){
					...
					dVal = val.dVal;
					element.title = val.tVal;
				}
				
				return element.value = dVal;
			}
			var oldVal = $("input.value", $this).val();
			$this.addClass("changed");//.fadeIn(100000000,function(){ $(this).removeClass("blink");})
			$this.trimError();
			var value = $this.attr("temp");
			var attr = $this.attr("attr");
			var handled = false;
			var dataVal = $this.getCustomDataDisplay(value);
			var tagType = $this.attr("tagType");
			if(dataVal!==""){
				$("input.display",$this).val("");
				$("div.display",$this).text(dataVal);
				$("div.display",$this).attr("oldval","");
				$("input.value", $this).val(value);
				handled = true;
			} else if($this.hasClass("toggle") || $this.hasClass("amount_toggle")){
				$(".option", $this).addClass("dn");
				var $sel = $(".option[data-value='"+value+"']:eq(0)", $this);
				if($sel && $sel.length)
					$sel.removeClass("dn");
				else {
					$(".option:eq(0)", $this).removeClass("dn");
					var value = $(".option:eq(0)", $this).attr("data-value");
				}
				$("input.toggleValue", $this).val(value);
				__retVal = $("input.toggleValue", $this).val();
			} else if($this.hasClass("date")){
				value = utils.custom.date.setValue($this,value);
			} else if($this.hasClass("instance_menu_item")){
				$(".title",$this).html("<a>" + value + "</a>").attr('title',value);
			} else if($this.hasClass("dropdown") || $this.hasClass("toggleinput")){
				value = utils.custom.dropdown.setValue($this,value,callOnChange, _dispVal);
				handled = true
			} else if($this.hasClass("inputbox")){
				value = utils.custom.inputbox.setValue($this,value);
			} else if($this.hasClass("amount")){
				value = utils.custom.amount.setValue($this,value);
			} else if($this.hasClass("checkbox")){
				value = utils.custom.checkbox.setValue($this,value);
			} else if($this.hasClass("tokeninput")){
				value = utils.custom.tokeninput.setValue($this,valObj,callOnChange);
			} else if($this.hasClass("stripbar")){
				utils.custom.stripbar.setValue($this,value);
			} else if($this.hasClass("label")){
				$this.text(value);
			}else if($this.hasClass("checkboxlist")){
				$this.text(value);
			}else if($this.hasClass("radiostrip")){
				$("input:radio", $this).removeAttr("checked");
				$("input:radio[value='" + value + "']", $this).attr("checked", true);
			}else if($this.hasClass("selection") || $this.hasClass("tabstrip")){
				utils.custom.selection.setValue($this,value);
			} else if(tagType && utils.custom[tagType] && utils.custom[tagType].setValue){
				__retVal = utils.custom[tagType].setValue($this,_value,callOnChange);
			}
			...
			if(attr==undefined){
				if(!handled){
					...
					if(callOnChange==true){ //&& (oldVal!=value)){
						$this.onChange();
					} else if(callOnChange===0){
						if(oldVal!=value) $this.onChange();
					}
				}
			} else 
				$("input.value", $this).attr(attr,value);

		});
	}
	if(__retVal!=undefined) return __retVal;
	return $("input.value", $widgets).val();
};
...
/*	if($this.hasClass("date")){
	...
	} else if( tagType &&  utils.custom[tagType] && utils.custom[tagType].getValue){
...
$.fn.onChange = function(cb,onerror){
	...
	if(cb==undefined){
		if($widget.hasClass("droption")){
			return $("input.value", $widget).change();
		}
		else if($widget.hasClass("tokeninput")){
			return $("input.input", $widget).change();
...
utils.selectNamespace("utils.custom.checkbox", function(checkbox){
...
utils.selectNamespace("utils.custom.checkboxlist", function(checkboxlist){
...
utils.selectNamespace("utils.custom.mouse", function(mouse){
