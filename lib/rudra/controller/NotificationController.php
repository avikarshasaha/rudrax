 ...
 */
include_once (RUDRA . "/controller/AbstractNotificationController.php");
...
class NotificationController extends AbstractNotificationController {
	...
	public function getLPollData($user, $handlerName) {
		...
		$counter = (int)$_REQUEST["counter"];
		$secCount = IDLE_WAIT;
		do {
			sleep(IDLE_TIME);
			$updates = $this->fetchAllNotifications($counter);
		} while (!$updates && ($secCount--)>0);
		
		header("HTTP/1.0 200");
		return sprintf ('{"time" : "%s", "counter" : "%d", start : %d, data : %s}'
				, date('d/m H:i:s'), $counter,$counter,json_encode($updates));
	}
